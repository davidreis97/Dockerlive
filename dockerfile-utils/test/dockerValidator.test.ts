...
function validateDockerfile(content: string, settings?: ValidatorSettings) {
    ...
    return validate(TextDocument.create("","",0,content), null, null, null, settings);
}
...
describe("Docker Validator Tests", function() {
    ...
    function createSingleQuotedJSONTests(instruction: string) {
        describe("JSON in single quotes", function() {
            ...
            it("ignore", function() {
                let diagnostics = validate(TextDocument.create("","",0,content), null, null, null, { instructionJSONInSingleQuotes: ValidationSeverity.IGNORE });
                assert.equal(diagnostics.length, 0);
            });

            it("warning", function() {
                let diagnostics = validate(TextDocument.create("","",0,content), null, null, null, { instructionJSONInSingleQuotes: ValidationSeverity.WARNING });
                assert.equal(diagnostics.length, 1);
                assertInstructionJSONInSingleQuotes(diagnostics[0], DiagnosticSeverity.Warning, 1, instructionLength + 1, 1, instructionLength + 22);
            });

            it("error", function() {
                let diagnostics = validate(TextDocument.create("","",0,content), null, null, null, { instructionJSONInSingleQuotes: ValidationSeverity.ERROR });
                assert.equal(diagnostics.length, 1);
